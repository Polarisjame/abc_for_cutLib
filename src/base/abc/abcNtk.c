...
void Abc_dup_recur(Abc_Obj_t * pRoot, Vec_Ptr_t * vNodes, int * vVisited){
    Abc_Obj_t * pFanin;
    int i;
    ...
    Abc_ObjForEachFanin(pRoot, pFanin, i){
        Abc_dup_recur(pFanin, vNodes, vVisited);
    }
...
Abc_Ntk_t * Abc_NtkDup_mine( Abc_Ntk_t * pNtk, int * fakeId2ID)
{
    // printf("Start Dup\n");
    Abc_Ntk_t * pNtkNew; 
    Abc_Obj_t * pObj, * pFanin, *pTemp0, *pTemp1;
    int i, k;
    if ( pNtk == NULL )
        return NULL;
    // start the network
    ...
    if ( Abc_NtkIsStrash(pNtk) )
    {
        Vec_Ptr_t * vNodes = Abc_DupDfs_mine(pNtk);
        // copy the AND gates
        Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pObj, i ){
                // printf("%d \n", pObj->Id);
                // printf("%d %d %d %d\n", Abc_ObjRegular(Abc_ObjChild0(pObj))->Id, Abc_ObjRegular(Abc_ObjChild0Copy(pObj))->Id, Abc_ObjRegular(Abc_ObjChild1(pObj))->Id, Abc_ObjRegular(Abc_ObjChild1Copy(pObj))->Id);
                // printf("%d \n", Abc_ObjRegular(Abc_ObjChild0Copy(pObj))->vFanins.nSize);
                // printf("%d \n", Abc_ObjRegular(Abc_ObjChild1Copy(pObj))->vFanins.nSize);
                // pTemp0 = Abc_ObjChild0Copy(pObj);
                // printf("pTemp0 %d\n", pTemp0->Id);
                // pTemp1 = Abc_ObjChild1Copy(pObj);
                // printf("pTemp1 %d\n", pTemp1->Id);
                
                pObj->pCopy = Abc_AigAnd( (Abc_Aig_t *)pNtkNew->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj) );
                pObj->pCopy->fakeId = pObj->Id;
                fakeId2ID[pObj->Id] = pObj->pCopy->Id;
                // printf("Finish And Copy\n");
        ...
        // relink the CO nodes
        Abc_NtkForEachCo( pNtk, pObj, i ){
            // printf("%d %d ", i, Abc_NtkCoNum(pNtk));
            // printf("%d ", pObj->Id);
            // printf("%d\n", Abc_ObjChild0Copy(pObj)->Id);
            Abc_ObjAddFanin( pObj->pCopy, Abc_ObjChild0Copy(pObj) );
            // printf("Finish CO Copy\n");
        }
        // get the number of nodes before and after
        if ( Abc_NtkNodeNum(pNtk) != Abc_NtkNodeNum(pNtkNew) )
            printf( "Warning: Structural hashing during duplication reduced %d nodes (this is a minor bug).\n",
