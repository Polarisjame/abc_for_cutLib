...
void * Abc_NodeGetCutsRecursiveMine( void * p, Abc_Obj_t * pObj, int fDag, int fTree )
{
    void * pList;
    if ( pList = Abc_NodeReadCuts( p, pObj ) )
        return pList;
    printf("%d %d\n", Abc_ObjFaninId0(pObj), Abc_ObjFaninId1(pObj));
    Abc_NodeGetCutsRecursiveMine( p, Abc_ObjFanin0(pObj), fDag, fTree );
    Abc_NodeGetCutsRecursiveMine( p, Abc_ObjFanin1(pObj), fDag, fTree );
    return Abc_NodeGetCuts( p, pObj, fDag, fTree );
}
