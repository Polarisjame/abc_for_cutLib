...
//            assert( 0 );
            ...
            Mio_Gate_t* pGateNew = (Mio_Gate_t*)pNodeNew->pData;
            return pNodeNew;
        }
    }
    ...
    printf("Create Gate Id: %d Node Id:%d \n", pRoot->Cell, pNodeNew->Id);
    return pNodeNew;
}
Abc_Obj_t * Abc_NodeFromMapPhase_rec( Abc_Ntk_t * pNtkNew, Map_Node_t * pNodeMap, int fPhase )
{
    ...
    unsigned uPhaseBest;
    int i, fInvPin, nLeaves, fRedirected;
    float tDelayMax;
    ...
    {
        fRedirected = Abc_FrameRedirectStdoutToLog( "abc_map.log", "a" );
        printf("ID:%d, AigID:%d, Area:%f, TruthTable:%u, %u, Cut Leaves: ",pNodeMap->Num, pNodeMap->p->pAigNodeIDs[pNodeMap->Num], pCutBest->M[fPhase].pSuperBest->Area, pCutBest->myTruthPos[0], pCutBest->myTruthPos[1]);
        for(int cut_ind=0; cut_ind<pCutBest->nLeaves; cut_ind++){
            Map_Node_t *pLeaf = pCutBest->ppLeaves[cut_ind];
            printf("%d ",pLeaf->Num);
        }
        printf("\n");
        if ( fRedirected )
    ...
    {
        fRedirected = Abc_FrameRedirectStdoutToLog( "abc_superCreate.log", "a" );
        printf("Creating Super: %d Node\n", pNodeMap->Num);
        tDelayMax = fmax(pSuperBest->tDelayMax.Rise, pSuperBest->tDelayMax.Fall);
        printf("Super delay: %f\n", tDelayMax);
...
Abc_Ntk_t * Abc_NtkFromMap( Map_Man_t * pMan, Abc_Ntk_t * pNtk, int fUseBuffs )
{
    char file_path[4096];
    FILE * file;
    if ( Abc_FrameMakeLogPath( file_path, sizeof(file_path), "abc_map.log" ) )
    {
        file = fopen( file_path, "w" );
        if ( file )
            fclose( file );
    }
    if ( Abc_FrameMakeLogPath( file_path, sizeof(file_path), "abc_superCreate.log" ) )
    {
        file = fopen( file_path, "w" );
        if ( file )
            fclose( file );
