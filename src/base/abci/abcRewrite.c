...
Rwr_ManAddTimeCuts( pManRwr, Abc_Clock() - clk );
    ...
    nNodes = Abc_NtkObjNumMax(pNtk);
    pProgress = Extra_ProgressBarStart( stdout, nNodes );
    Rwr_printCut( pManCut, pNtk, "cut_trave.log" );
    return RetValue;
}

int Abc_NtkRewrite_mine( Abc_Ntk_t * pNtk, int fUpdateLevel, int fUseZeros, int fVerbose, int fVeryVerbose, int fPlaceEnable )
{
    extern int           Dec_GraphUpdateNetwork( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain );
    ProgressBar * pProgress;
    Cut_Man_t * pManCut;
    Rwr_Man_t * pManRwr, *pManRwrCopy;
    Abc_Obj_t * pNode;
//    Vec_Ptr_t * vAddedCells = NULL, * vUpdatedNets = NULL;
    Dec_Graph_t * pGraph;
    int i, nNodes, nGain, fCompl, RetValue = 1;
...
Rwr_ManAddTimeCuts( pManRwr, Abc_Clock() - clk );
    ...
    // resynthesize each node once
    pManRwr->nNodesBeg = Abc_NtkNodeNum(pNtk);
    pManRwrCopy->nNodesBeg = Abc_NtkNodeNum(pNtk);
    nNodes = Abc_NtkObjNumMax(pNtk);
    pProgress = Extra_ProgressBarStart( stdout, nNodes );

    int PORT = 65436;
    FILE *fp = Abc_FrameOpenLogFile( "cut_trave.log", "w" );
    if ( fp )
        fclose(fp);
    fp = Abc_FrameOpenLogFile( "rw_log.log", "w" );
    if ( fp )
        fclose(fp);
    fp = Abc_FrameOpenLogFile( "RwrStrucutOri.log", "w" );
    if ( fp )
        fclose(fp);
    fp = Abc_FrameOpenLogFile( "RwrStrucutUpdated.log", "w" );
    if ( fp )
        fclose(fp);
    fp = Abc_FrameOpenLogFile( "cut_trave_update.log", "w" );
    if ( fp )
        fclose(fp);
    ...
    Abc_NtkForEachNode( pNtk, pNode, i )
    {
        ...
        if ((double)rand() / RAND_MAX < 0.3) 
            nGain = Rwr_NodeRewrite_mine(sock, pManRwr, pManRwrCopy, pManCut, pNode, pNtk, i, fUpdateLevel, fUseZeros, fPlaceEnable );
        else
            nGain = Rwr_NodeRewrite( pManRwr, pManCut, pNode, fUpdateLevel, fUseZeros, fPlaceEnable);
        Rwr_print("Done One Node\n");
        char json_data[100] = "{\"stage\": \"done node\"}";
