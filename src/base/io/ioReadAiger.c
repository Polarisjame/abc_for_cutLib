...
Abc_Ntk_t * Io_ReadAiger( char * pFileName, int fCheck )
{
    ...
    int nFileSize = -1, iTerm, nDigits, i, fRedirected;
...
//        printf( "Creating latch %s with input %d and output %d.\n", Abc_ObjName(pObj), pNode0->Id, pNode1->Id );
    ...
    fRedirected = Abc_FrameRedirectStdoutToLog( "abc_aig.log", "w" );
    for ( i = 0; i < nAnds; i++ )
    {
...
//        assert( uLit1 > uLit0 );
        ...
        Abc_Obj_t * pAnd = Abc_AigAnd((Abc_Aig_t *)pNtkNew->pManFunc, pNode0, pNode1);
        printf("%d %d %d %d %d\n", Abc_ObjRegular(pNode0)->Id, Abc_ObjIsComplement(pNode0),  Abc_ObjRegular(pNode1)->Id, Abc_ObjIsComplement(pNode1), pAnd->Id);
    ...
    if ( pContents[3] == ' ' ) // standard AIGER
    {
        ...
        Abc_NtkForEachPo( pNtkNew, pObj, i )
        {
            ...
            pNode0 = Abc_ObjNotCond( (Abc_Obj_t *)Vec_PtrEntry(vNodes, uLit0 >> 1), (uLit0 & 1) );//^ (uLit0 < 2) );
            Abc_ObjAddFanin( pObj, pNode0 );
            printf("%d %d %d\n", Abc_ObjRegular(pNode0)->Id, Abc_ObjIsComplement(pNode0), pObj->Id);
        }
    }
    else
    {
        ...
        Abc_NtkForEachPo( pNtkNew, pObj, i )
        {
            ...
            pNode0 = Abc_ObjNotCond( (Abc_Obj_t *)Vec_PtrEntry(vNodes, uLit0 >> 1), (uLit0 & 1) );
            Abc_ObjAddFanin( pObj, pNode0 );
            printf("Get Unnormal File!\n%d %d\n", Abc_ObjRegular(pNode0)->Id, pObj->Id);
        }
        Vec_IntFree( vLits );
    }
