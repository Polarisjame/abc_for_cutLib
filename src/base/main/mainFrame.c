...
char *      Abc_FrameReadLogDir()                            { return s_GlobalFrame->pLogDir;         }
void        Abc_FrameSetLogDir( const char * pDir )          { ABC_FREE(s_GlobalFrame->pLogDir); s_GlobalFrame->pLogDir = Extra_UtilStrsav( pDir ? pDir : "." ); }
int         Abc_FrameMakeLogPath( char * pBuffer, int nBufferSize, const char * pFileName )
{
    char * pLogDir;
    int nChars;
    if ( s_GlobalFrame == NULL || pBuffer == NULL || nBufferSize <= 0 || pFileName == NULL )
        return 0;
    pLogDir = s_GlobalFrame->pLogDir ? s_GlobalFrame->pLogDir : ".";
    nChars = snprintf( pBuffer, nBufferSize, "%s%s%s", pLogDir,
        (pLogDir[0] && pLogDir[strlen(pLogDir) - 1] == '/') ? "" : "/", pFileName );
    if ( nChars < 0 || nChars >= nBufferSize )
        return 0;
    return 1;
}
FILE *      Abc_FrameOpenLogFile( const char * pFileName, const char * pMode )
{
    char Buffer[4096];
    if ( pMode == NULL || !Abc_FrameMakeLogPath( Buffer, sizeof(Buffer), pFileName ) )
        return NULL;
    return fopen( Buffer, pMode );
}
int         Abc_FrameRedirectStdoutToLog( const char * pFileName, const char * pMode )
{
    char Buffer[4096];
    if ( pMode == NULL || !Abc_FrameMakeLogPath( Buffer, sizeof(Buffer), pFileName ) )
        return 0;
    return freopen( Buffer, pMode, stdout ) != NULL;
}
int         Abc_FrameRestoreStdout()
{
#ifdef WIN32
    return freopen( "CON", "w", stdout ) != NULL;
#else
    return freopen( "/dev/tty", "w", stdout ) != NULL;
#endif
}
