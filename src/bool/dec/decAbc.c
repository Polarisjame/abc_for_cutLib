...
Abc_Obj_t * Dec_GraphToNetwork( Abc_Ntk_t * pNtk, Dec_Graph_t * pGraph )
{
    ...
    {
        // printf("%d %d %d\n", pNode->eEdge0.Node, pNode->eEdge1.Node, i);
...
int Dec_GraphUpdateNetwork_mine( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain )
{
    extern Abc_Obj_t *    Dec_GraphToNetwork( Abc_Ntk_t * pNtk, Dec_Graph_t * pGraph );
    Abc_Obj_t * pRootNew, * pRootNewR, * pNode;
    Abc_Ntk_t * pNtk = pRoot->pNtk;
    Vec_Ptr_t * vNodes = Vec_PtrAlloc( Abc_NtkObjNumMax(pNtk) );
    int nNodesNew, nNodesOld, RetValue, k;

    nNodesOld = Abc_NtkNodeNum(pNtk);
    // create the new structure of nodes
    pRootNew = Dec_GraphToNetwork( pNtk, pGraph );
    pRootNewR = Abc_ObjRegular(pRootNew);
    if(pRootNewR == Abc_ObjRegular(pRoot))
    {
        // Abc_NtkDeleteObj(pRootNew);
        return -1;
    }
    Abc_NodeCollectFanouts( pRoot, vNodes );
    Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pNode, k ){
        if( Abc_ObjRegular(pNode) == pRootNewR )
            return -1;
    }
    // Vec_PtrPush(vSubNodes, pRootNew);
    // remove the old nodes
    RetValue = Abc_AigReplace( (Abc_Aig_t *)pNtk->pManFunc, pRoot, pRootNew, fUpdateLevel );
    // compare the gains
    nNodesNew = Abc_NtkNodeNum(pNtk);
