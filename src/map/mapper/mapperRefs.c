...
***********************************************************************/
float Map_MappingGetArea( Map_Man_t * pMan, int fDisplay )
{
    Map_Node_t * pNode;
    Map_Cut_t *cutBest;
    int cut_leaves[6];
    ...
    for ( i = 0; i < pMan->vMapObjs->nSize; i++ )
    {
        ...
        if ( Map_NodeIsAnd(pNode) )
        {
            ...
            if ( pNode->pCutBest[0] && (pNode->nRefAct[0] > 0 || pNode->pCutBest[1] == NULL) )
            {
                ...
                cutBest = pNode->pCutBest[0];
                if ( pMan->fUseProfile )
                    Mio_GateIncProfile2( pNode->pCutBest[0]->M[0].pSuperBest->pRoot );
            }
            if (fDisplay && used){
                printf("ID:%d, AigID:%d, Cnt:%d Area:%f totalArea:%f Cut Leaves: ",pNode->Num, pNode->p->pAigNodeIDs[pNode->Num],++cnt, cutBest->M[0].pSuperBest->Area, Area);
                for(int cut_ind=0; cut_ind<cutBest->nLeaves; cut_ind++){
                    Map_Node_t *pLeaf = cutBest->ppLeaves[cut_ind];
                    cut_leaves[cut_ind] = pLeaf->Num;
                    printf("%d ",cut_leaves[cut_ind]);
                }
                printf("\n");
            }
            used = 0;
            ...
            if ( pNode->pCutBest[1] && (pNode->nRefAct[1] > 0 || pNode->pCutBest[0] == NULL) )
            {
                ...
                cutBest = pNode->pCutBest[1];
                if ( pMan->fUseProfile )
                    Mio_GateIncProfile2( pNode->pCutBest[1]->M[1].pSuperBest->pRoot );
            }
            // if (fDisplay && used){
            //     printf("ID:%d, AigID:%d, Cnt:%d Area:%f totalArea:%f Cut Leaves: ",pNode->Num, pNode->p->pAigNodeIDs[pNode->Num],++cnt, cutBest->M[1].pSuperBest->Area, Area);
            //     for(int cut_ind=0; cut_ind<cutBest->nLeaves; cut_ind++){
            //         Map_Node_t *pLeaf = cutBest->ppLeaves[cut_ind];
            //         cut_leaves[cut_ind] = pLeaf->Num;
            //         printf("%d ",cut_leaves[cut_ind]);
        ...
        if ( (pNode->pCutBest[0] == NULL && pNode->nRefAct[0] > 0) || 
             (pNode->pCutBest[1] == NULL && pNode->nRefAct[1] > 0) ){
                ...
                if (fDisplay)
                    printf("Inverter Cnt:%d Area:%f totalArea:%f\n",++cnt, pMan->pSuperLib->AreaInv, Area);
             }
    }
